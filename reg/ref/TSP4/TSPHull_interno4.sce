...
for k=1:size(Areas_opt,1)
      ...
      plot(W(k).coord(:,1),W(k).coord(:,2),'x')
      plot([W(k).coord(H(k).idxW(size(H(k).idxW,2)),1);,W(k).coord(H(k).idxW,1)],[W(k).coord(H(k).idxW(size(H(k).idxW,2)),2);,W(k).coord(H(k).idxW,2)],'b')
      plot(W(k).coord(B(k).idxW,1),W(k).coord( B(k).idxW,2),'ro')
             
      //Encontra a casca convexa dos pontos internos
      B(k).idxW = [1 B(k).idxW];//este caso inclui de novo a origem. Se isso funcionar nao precisa mais reorganizar os pontos
      B(k).n = B(k).n+1         //lembrar de retirar isso se eu nao quiser fazer assim
      ...
      //calcula os headings e o tamanho do vertices, sendo o primeiro o de entrada e o segundo o de saida
      dists= zeros(1,P(k).n)
    ...
    while B(k).n > 1 //por causa de B ter sempre a origem, senao seria > 0
    // Levanta o conjunto de pontos visiveis
        if HB(k).n >= 3 then  //so faz se tiver dois pontos alem da propria origem
            //Pega a primeira triade a partir do segundo ponto de HB, pois o 1o é a origem
             ...
             HBL=W(k).coord(HB(k).idxW(i-1),1:2) //HBL=W(k).coord(HB(k).idxW(HB(k).n),1:2) se tirar a origem
            ...
            //é visivel os pontos que fizerem curva para a direita
             ...
             for j=1:size(visib,2)-1
                  //calcula a metrica, que no caso é a diferenca de cauchy + a diferenca entre as mudancas de headings
    ...
    plot(P(k).coord(:,1),P(k).coord(:,2),'c')
        


    if B(k).n > 0, plot(W(k).coord(B(k).idxW,1),W(k).coord(B(k).idxW,2),'mo'), end
    if HB(k).n > 0, plot(W(k).coord(HB(k).idxW,1),W(k).coord(HB(k).idxW,2),'r'), end
    if R(k).n > 0, plot(W(k).coord(R(k).idxW,1),W(k).coord(R(k).idxW,2),'g+'), end

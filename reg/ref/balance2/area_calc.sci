...
for i=1:n
  for j=1:n_pts
    if not_indi(j)==indi(i),not_indi_idx(j)=0,end
  end
end
...
//disp('pause',p_perimetro),pause

//p_area1 = p_area1+2*sum(dists(not_indi));

p_area1 = p_area2 + p_perimetro+2*sum(dists(not_indi))//

//quero testar outra funcao que é a soma das distancias, pois ela é
//simples e considera os pontos internos
